...
__all__ = ['FastDSAGCN', 'get_fast_scnn']
...
class DSSGCModule(nn.Module):
    def __init__(self, in_channels, d=1, kSize=1, dkSize=3):
        super(DSSGCModule,self).__init__()

        self.bn_relu_1 = BNPReLU(in_channels)
        self.bottleneck = nn.Conv2d(in_channels, in_channels, kernel_size=1)
...
class DownSamplingBlock(nn.Module):
    def __init__(self, in_channels, dw_channels=3, out_channels=64, **kwargs):
        super().__init__()
       
        self.Conv = ConvBNPreLU(3, 32, 3, 2,0, 3)
...
class HHDSAModule(nn.Module):
    def __init__(self,in_channels, kSize=1, dkSize=1):
        super(HHDSAModule,self).__init__()
        ...
        self.dconv6 = ConvBNPreLU(in_channels, in_channels, 3, 1, padding=3, dilation=3, bn_act=False)
        self.dconv7 = ConvBNPreLU(in_channels, in_channels, 3, 1, padding=3, dilation=3, bn_act=False)
    ...
    def forward(self, input_):
        ...
        br5 = self.dconv6(output)
        br6 = self.dconv7(output)

        out1 = torch.mul(br1, br2)
        out1 = self.sigmoid(out1)
        out4 = torch.mul(br5, br6)
        out4 = self.sigmoid(out4)

        out2 = torch.mul(out1, br3)
        # out2 = self.sigmoid(out2)
        out5 = torch.mul(out4, br3)

        out6 = torch.add(out4, out5)
        out3 = torch.add(output, out6)
...
class DualAttention(nn.Module):
    ...
    def __init__(self, in_channels,reduction=2):
        ...
        self.spatial = HHDSAModule(in_channels)
...
class FastDSAGCN(nn.Module):
    def __init__(self, classes=11, block_1=3, block_2=4, block_3=4):
        ...
        self.DSSGC_Block_1 = nn.Sequential()
        for i in range(0, block_1):
            self.DSSGC_Block_1.add_module("DSSGC_Module_1_" + str(i),DSSGCModule(64, d=1))
        ...
        self.DSSGC_Block_2 = nn.Sequential()
        for i in range(0, block_2):
            self.DSSGC_Block_2.add_module("DSSGC_Module_2_" + str(i),
                                       DSSGCModule(128, d=dilation_block2[i]))
        ...
        self.DSSGC_Block_3 = nn.Sequential()
        for i in range(0, block_3):
            self.DSSGC_Block_3.add_module("DSSGC_Module_3_" + str(i),
                                       DSSGCModule(128, d=dilation_block3[i]))
    ...
    def forward(self, input):
        ...
        output1 = self.DSSGC_Block_1(output1)
        ...
        output2 = self.DSSGC_Block_2(output2)
        ...
        # Block 3
        output3 = self.DSSGC_Block_3(output2)
...
def get_fast_scnn(dataset='citys', pretrained=False, root='./weights', map_cpu=False, **kwargs):
    ...
    model = FastDSAGCN(datasets[dataset].NUM_CLASS, **kwargs)
    if pretrained:
        if(map_cpu):
...
if __name__ == '__main__':
    ...
    model = get_fast_scnn('citys') # , args=self.aux
    outputs = model(img)
